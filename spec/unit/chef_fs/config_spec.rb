...
describe Chef::ChefFS::Config do
  ...
  describe "local FS configuration" do
    ...
    it "sets the correct nodes path on the local FS object" do
      expect(local_fs.child_paths["nodes"]).to eq(["/base_path/nodes"])
    end

    it "sets the correct cookbook path on the local FS object" do
      expect(local_fs.child_paths["cookbooks"]).to eq(["/base_path/cookbooks"])
    end

    it "sets the correct data bag path on the local FS object" do
      expect(local_fs.child_paths["data_bags"]).to eq(["/base_path/data_bags"])
    end

    it "sets the correct environment path on the local FS object" do
      expect(local_fs.child_paths["environments"]).to eq(["/base_path/environments"])
    end

    it "sets the correct role path on the local FS object" do
      expect(local_fs.child_paths["roles"]).to eq(["/base_path/roles"])
    end

    it "sets the correct user path on the local FS object" do
      expect(local_fs.child_paths["users"]).to eq(["/base_path/users"])
    end

    it "sets the correct policy path on the local FS object" do
      expect(local_fs.child_paths["policies"]).to eq(["/base_path/policies"])
    end
  end
end
